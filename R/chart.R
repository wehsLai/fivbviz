...
title.font <- list(size = 18)

axis.font <- list(size = 14)
...
playerXyChart <-
  function(data,
           ...
           ytitle = "") {
    if (missing(data))
      stop("playerXyChart must input data")
...
playerBarChart <-
  function(data,
           ...
           ytitle = "") {
    if (missing(data))
      stop("playerBarChart must input data")
...
teamXyChart <-
  function(data,
           ...
           ytitle = "") {
    if (missing(data))
      stop("teamXyChart must input data")
    ...
    sizerefx <-
      7 * diff(range(data$x)) / (desired_maximum_marker_size ** 2)
    sizerefy <-
      7 * diff(range(data$y)) / (desired_maximum_marker_size ** 2)
