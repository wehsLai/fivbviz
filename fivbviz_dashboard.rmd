...
<font size="2">1. Select a tournament >> OPEN</font>

<font size="2">2. (option)Select Round >> Filter</font>
...
observe({
  if (!is.null(rv$fds) && all(map_lgl(rv$fds$statistics, ~nrow(.x) > 0)) == TRUE) {
    type <- "c"
    isTeam <- FALSE
...
matchesUI("matches")
observe({
  if (!is.null(rv$fds) && nrow(rv$fds$matches) > 0) {
...
rosterUI("roster")
observe({
  if (!is.null(rv$fds) && nrow(rv$fds$players) > 0) {
    rosterServer("roster")
  }
})
```
